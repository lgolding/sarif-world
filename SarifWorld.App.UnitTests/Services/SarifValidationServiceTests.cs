...
namespace SarifWorld.App.Services
{
    public class SarifValidationServiceTests
    {
        ...
        [Fact]
        public void SarifValidationService_WhenInputFileHasSchemaErrors_ReportsValidationResults()
        {
            ValidationResult validationResult = ValidateFile("SchemaErrors.sarif");
        ...
        [Fact]
        public void SarifValidationService_WhenInputFileHasSarifWarnings_ReportsValidationResults()
        {
            ValidationResult validationResult = ValidateFile("SarifWarnings.sarif");

            var results = validationResult.ValidationLog.Runs[0].Results;
            results.Should().NotBeEmpty();

            validationResult.ExitCode.Should().Be((int)RuntimeConditions.OneOrMoreWarningsFired);
            validationResult.ErrorMessage.Should().BeEmpty();
        }
        ...
        private ValidationResult ValidateFile(string fileName)
        {
            ...
            // of the input and output files from GUIDs. The tests relies on the fact that the
            ...
            //
            // In contrast, in these tests, the SarifValidationService _creates_ the input file
            ...
            return service.ValidateFile(fileName, fileContents);
        }
    }
}
